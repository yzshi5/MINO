...
class MINO(nn.Module):
    def __init__(self, conditioner, encoder, processor, decoder):
        super().__init__()
    ...
    def forward(
        ...
        if timestep.dim() == 0 or timestep.numel() == 1:
            timestep = torch.ones(latent.shape[0], device=timestep.device) * timestep
            
        latent = self.processor(timestep, latent)
