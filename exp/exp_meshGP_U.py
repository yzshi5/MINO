...
parser = argparse.ArgumentParser('Training MINO-U on Mesh GP dataset')
...
data_path = args.data_path + '/train_fish.npy'
data_test_path = args.data_path + '/test_fish.npy'
pos_path = args.data_path + '/fish_mesh.npy'
...
"""
# visualization
# triang for plot "/fish_triang.npy"
def plot_cyl(x, y,  triang, field_val, vmin=None, vmax=None, save_fig=False, input_text='v1', random_subsample=False, subsample_ratio=1):
    
    # x, y comes from pos_path, field_val is the function value
    ...
    # wireframewireframe cmap='jet' cmap='Spectral'coolwarm'
    tcf = ax.tricontourf(triang, field_val, levels=50, cmap='Spectral', extend='max', vmin=vmin, vmax=vmax)  # filled field
    ax.triplot(triang, lw=0.1, c='k')   
    #fig.colorbar(tcf, ax=ax, label='Field value')
    ax.set_aspect('equal')
    #ax.set_xlabel('x'), ax.set_ylabel('y')
    #ax.set_title('Fluid‑field snapshot on supplied unstructured mesh')
    plt.tight_layout()
    plt.xticks([])
    plt.yticks([])
